                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <p class="flex items-baseline gap-2">
                                    <span class="text-xl font-medium text-ink tnum">@price($deal->group_price_sen)</span>
                            ...
                            <button type="button" wire:click="start({{ $deal->id }})" wire:loading.attr="disabled" wire:target="start"
                                    class="inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] bg-emerald px-4 text-sm font-medium text-white transition-colors hover:bg-emerald-deep disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald focus-visible:ring-offset-1">
                                {{ __('Start a group') }}
                            </button>
                        </div>

                        @if ($teams->isNotEmpty())
                            <p class="mt-3 text-[11px] font-medium uppercase tracking-[0.06em] text-ink-faint">{{ __('Join an open group') }}</p>
                                ...
                                @foreach ($teams as $team)
                                    <li class="flex items-center justify-between gap-3 rounded-[var(--radius-control)] border border-line bg-paper px-3 py-2">
                                        ...
                                        <a href="{{ route('group-buy.team', $team->code) }}" wire:navigate
                                           class="shrink-0 text-[13px] font-medium text-emerald hover:text-emerald-deep">{{ __('Join') }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        @endif
