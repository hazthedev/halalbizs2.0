        ...
        <div class="p-5">
            @if ($product !== null)
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <a href="{{ route('product.show', $product->slug) }}" wire:navigate class="line-clamp-2 text-sm font-medium text-ink hover:text-emerald">
                            {{ $product->getTranslation('name', app()->getLocale()) }}
                        </a>
                        <p class="mt-1 text-lg font-medium text-ink tnum">@price($deal->group_price_sen)
            ...
            <div class="mt-5">
                @if ($unlocked)
                    <div class="rounded-[var(--radius-control)] bg-emerald-tint px-4 py-3 text-center text-[13px] font-medium text-emerald">
                    ...
                    @if ($product !== null)
                        <a href="{{ route('product.show', $product->slug) }}" wire:navigate
                           class="mt-3 inline-flex min-h-11 w-full items-center justify-center rounded-[var(--radius-control)] bg-emerald px-4 text-sm font-medium text-white transition-colors hover:bg-emerald-deep">
                ...
                @elseif ($isMember)
                    ...
                    <div class="mt-3 flex gap-2" x-data="{ copied: false }">
                        ...
                        <button type="button" x-on:click="navigator.clipboard.writeText($refs.link.value); copied = true; setTimeout(() => copied = false, 2000)"
                                class="inline-flex min-h-11 shrink-0 items-center rounded-[var(--radius-control)] border border-line-strong bg-surface px-4 text-sm font-medium text-ink hover:border-ink">
