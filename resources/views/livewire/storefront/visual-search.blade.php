<div class="mx-auto w-full max-w-7xl px-4 py-8 lg:py-12">
    ...
    <div class="mt-5 max-w-xl">
        <label for="visual-upload"
            ...
            <span class="text-sm font-medium text-ink">{{ __('Choose a photo') }}</span>
        ...
        @if ($image)
            <div class="mt-3 flex items-center gap-3">
                ...
                <button type="button" wire:click="clear" class="text-[13px] font-medium text-ink-soft hover:text-danger">{{ __('Clear') }}</button>
            </div>
        @endif
    </div>
    ...
    @elseif ($products->isNotEmpty())
        <h2 class="mt-8 font-display text-lg font-medium">{{ __('Visually similar') }}</h2>
