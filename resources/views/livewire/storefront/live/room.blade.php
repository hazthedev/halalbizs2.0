<div wire:poll.15s class="mx-auto w-full max-w-7xl px-4 py-6">
    ...
    <div class="flex flex-wrap items-center gap-3">
        @if ($isLive)
            <span class="inline-flex items-center gap-1.5 rounded-full bg-danger px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.06em] text-white">
        ...
        @endif
        <h1 class="font-display text-xl font-medium">{{ $session->title }}</h1>
        <a href="{{ route('store.show', $session->store->slug) }}" wire:navigate class="text-[13px] font-medium text-emerald hover:text-emerald-deep">{{ $session->store?->name }}</a>
    </div>
    ...
    <div class="mt-4 grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
        ...
        <div>
            <div class="relative flex aspect-video items-center justify-center overflow-hidden rounded-[var(--radius-card)] border border-emerald-edge bg-emerald-night">
                ...
                @else
                    <div class="text-center text-on-dark/70">
                ...
                <div class="mt-4 flex flex-wrap items-center gap-4 rounded-[var(--radius-card)] border border-emerald/40 bg-emerald-tint/40 p-4">
                    <span class="size-20 shrink-0 overflow-hidden rounded-[var(--radius-card)] bg-paper">
                        @if ($featured->getFirstMediaUrl('images', 'thumb'))
                            <img src="{{ $featured->getFirstMediaUrl('images', 'thumb') }}" alt="" class="size-full object-contain">
                        @endif
                    </span>
                    <div class="min-w-0 flex-1">
                        <p class="text-[11px] font-medium uppercase tracking-[0.06em] text-emerald">{{ __('Featured now') }}</p>
                    ...
                    @else
                        <a href="{{ route('product.show', $featured->slug) }}" wire:navigate class="inline-flex min-h-11 shrink-0 items-center justify-center rounded-[var(--radius-control)] border border-emerald bg-surface px-5 text-sm font-medium text-emerald hover:bg-emerald hover:text-white">{{ __('Choose options') }}</a>
                    @endif
                </div>
            @endif
        </div>
        ...
        <div class="space-y-4">
            @if ($session->voucher_code)
                <div class="flex items-center justify-between gap-2 rounded-[var(--radius-card)] border border-dashed border-brass/50 bg-brass/10 px-4 py-3">
                    <div class="min-w-0">
                        ...
                        <p class="font-mono text-sm font-medium text-ink">{{ $session->voucher_code }}</p>
                    </div>
            ...
            @if ($sold->isNotEmpty())
                <div class="rounded-[var(--radius-card)] border border-line bg-surface p-3 shadow-soft">
                    <p class="mb-2 text-[11px] font-medium uppercase tracking-[0.06em] text-ink-faint">{{ __('Just sold') }}</p>
            ...
            {{-- Product rail --}}
            <div class="rounded-[var(--radius-card)] border border-line bg-surface p-3 shadow-soft">
                <p class="mb-2 text-[11px] font-medium uppercase tracking-[0.06em] text-ink-faint">{{ __('Shop the stream') }}</p>
                <ul class="space-y-2">
                    @foreach ($session->products as $product)
                        ...
                        <li class="flex items-center gap-2.5 rounded-[var(--radius-control)] border border-line p-2">
                            <a href="{{ route('product.show', $product->slug) }}" wire:navigate class="size-12 shrink-0 overflow-hidden rounded-[var(--radius-control)] bg-paper">
                                @if ($product->getFirstMediaUrl('images', 'thumb'))
                                    <img src="{{ $product->getFirstMediaUrl('images', 'thumb') }}" alt="" class="size-full object-contain">
                                @endif
                            </a>
                            <div class="min-w-0 flex-1">
