<div class="mx-auto max-w-7xl px-4 py-6" x-data="{ filtersOpen: false }" x-on:keydown.escape.window="filtersOpen = false">
    ...
    <div class="flex flex-wrap items-end justify-between gap-3">
        <div>
            <h1 class="font-display text-2xl font-bold">
                @if ($isSearch)
                    {{ trim($q) === '' ? __('Search') : __('Search: :term', ['term' => trim($q)]) }}
                @else
                    {{ $rootCategory->getTranslation('name', app()->getLocale()) }}
                @endif
            </h1>
        ...
        <div class="ml-auto">
            ...
            <select
                ...
                class="min-h-11 cursor-pointer rounded-lg border border-line-strong bg-surface px-3 text-sm text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald"
            >
                @if ($isSearch && trim($q) !== '')
    ...
    <div class="mt-6 flex items-start gap-8">
        ...
        <div class="min-w-0 flex-1">
            ...
            <button
                ...
                class="mb-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-line-strong bg-surface px-4 text-sm font-semibold text-ink lg:hidden"
            >
                ...
                @if ($products->isEmpty())
                    ...
                    <div class="rounded-[10px] border border-line bg-surface px-6 py-16 text-center">
                        <p class="font-display text-2xl font-bold">
                            @if ($isSearch && trim($q) !== '')
                                {{ __('No results for ":term".', ['term' => trim($q)]) }}
                            @else
                                {{ __('Nothing here yet.') }}
                            @endif
                        </p>
                        <p class="mt-2 text-sm text-ink-soft">
                            ...
                            @elseif ($isSearch)
                                {{ __('Check the spelling or try a more general term.') }}
                            @else
                                {{ __('Products will appear here as soon as sellers list them.') }}
                            @endif
                        </p>
                        <div class="mt-6">
                            @if (count($chips) > 0)
    ...
    <div x-show="filtersOpen" x-cloak class="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="{{ __('Filters') }}">
        ...
        <div
            ...
            class="absolute inset-x-0 bottom-0 flex max-h-[85vh] flex-col rounded-t-[10px] bg-surface shadow-xl"
        >
            <div class="flex items-center justify-between border-b border-line px-4 py-3">
                <p class="font-display text-lg font-bold">{{ __('Filters') }}</p>
                <button type="button" x-on:click="filtersOpen = false" class="flex size-11 items-center justify-center rounded-lg text-ink-soft hover:text-ink" aria-label="{{ __('Close filters') }}">
