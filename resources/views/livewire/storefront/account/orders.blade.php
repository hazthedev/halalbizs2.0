            ...
            <nav class="flex min-w-max border-b border-line" aria-label="{{ __('Order status') }}">
                @foreach ($tabs as $key => $label)
                    ...
                    <button type="button" wire:click="setTab('{{ $key }}')"
                        ...
                        {{ $label }}
                        @if (($counts[$key] ?? 0) > 0)
                            <span class="rounded-full bg-emerald-tint px-1.5 py-0.5 text-[11px] font-medium leading-none text-emerald">{{ $counts[$key] }}</span>
                        @endif
                    </button>
                @endforeach
            </nav>
            ...
            @if ($tab === 'to-pay')
                    ...
                    <x-ui.card class="p-4" wire:key="order-{{ $order->id }}">
                        ...
                        <div class="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-line pt-3">
                            <p class="text-sm text-ink-soft">
                                {{ __('Total') }}
                                <span class="ml-1 text-base font-medium text-ink" style="font-feature-settings: 'tnum'">@money($order->grand_total_sen)</span>
                            </p>
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <a href="{{ route('payments.ipay88.pay', $order) }}"
                                   class="inline-flex min-h-11 items-center justify-center gap-2 rounded-[var(--radius-control)] bg-emerald px-4 py-2.5 text-sm font-medium text-white transition-colors duration-150 hover:bg-emerald-deep">
                                    {{ __('Pay now') }}
                                </a>
                            </div>
                        </div>
                ...
                @empty
                    <x-ui.card class="px-6 py-16 text-center">
                        @if (trim($search) !== '')
                            <p class="font-display text-[22px] font-medium">{{ __('No orders match ":term".', ['term' => trim($search)]) }}</p>
                ...
                @forelse ($subOrders as $subOrder)
                    ...
                    <x-ui.card wire:key="sub-order-{{ $subOrder->id }}" class="overflow-hidden">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3 border-t border-line px-4 py-3">
                            <p class="text-sm text-ink-soft">
                                {{ __('Order total') }}
                                <span class="ml-1 text-base font-medium text-ink" style="font-feature-settings: 'tnum'">@money($subOrder->total_sen)</span>
                            </p>
                            <div class="flex flex-wrap items-center gap-2">
                ...
                @empty
                    <x-ui.card class="px-6 py-16 text-center">
                        @if (trim($search) !== '')
                            ...
                            <x-ui.button wire:click="$set('search', '')" class="mt-6">{{ __('Clear search') }}</x-ui.button>
                        @elseif ($tab === 'to-ship')
                            <p class="font-display text-[22px] font-medium">{{ __('No orders to ship.') }}</p>
