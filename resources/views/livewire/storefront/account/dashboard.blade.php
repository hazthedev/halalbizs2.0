            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-3 lg:grid-cols-5">
                    <x-ui.card class="p-4">
                        ...
                        <p class="mt-1 font-display text-[28px] font-medium leading-tight tabular-nums text-emerald">@money($totalSpentSen)</p>
                    ...
                    <x-ui.card class="p-4" x-data="countUp({{ $ordersPlaced }})">
                        ...
                        <p class="mt-1 font-display text-[28px] font-medium leading-tight tabular-nums" x-text="display.toLocaleString()">{{ number_format($ordersPlaced) }}</p>
                    ...
                    <x-ui.card class="p-4" x-data="countUp({{ $itemsBought }})">
                        ...
                        <p class="mt-1 font-display text-[28px] font-medium leading-tight tabular-nums" x-text="display.toLocaleString()">{{ number_format($itemsBought) }}</p>
                    ...
                    <x-ui.card class="p-4" x-data="countUp({{ $reviewsWritten }})">
                        ...
                        <p class="mt-1 font-display text-[28px] font-medium leading-tight tabular-nums" x-text="display.toLocaleString()">{{ number_format($reviewsWritten) }}</p>
                    ...
                    <x-ui.card class="p-4" x-data="countUp({{ $wishlistSaved }})">
                        ...
                        <p class="mt-1 font-display text-[28px] font-medium leading-tight tabular-nums" x-text="display.toLocaleString()">{{ number_format($wishlistSaved) }}</p>
                    ...
                    @foreach ($quickStatus as $shortcut)
                        <a href="{{ $shortcut['url'] }}" wire:navigate wire:key="quick-{{ $shortcut['key'] }}"
                           class="group flex min-h-11 items-center justify-between rounded-[var(--radius-card)] border border-line bg-surface p-4 shadow-soft hover:border-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald">
                            ...
                            <span class="font-display text-[22px] font-medium tabular-nums {{ $shortcut['count'] > 0 ? 'text-ink' : 'text-ink-faint' }}">{{ number_format($shortcut['count']) }}</span>
                        </a>
                    @endforeach
                </div>
                ...
                <div class="grid gap-4 lg:grid-cols-2">
                    {{-- Spend over time --}}
                    <x-ui.card class="p-4">
                        <h2 class="text-sm font-medium">{{ __('Spend over time') }}</h2>
                            ...
                            <li wire:key="recent-{{ $subOrder->id }}">
                                <a href="{{ route('account.orders.show', $subOrder) }}" wire:navigate
                                   class="flex min-h-11 items-center gap-3 px-4 py-3 hover:bg-paper focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald">
                                    ...
                                    <span class="font-medium tabular-nums whitespace-nowrap text-ink">@money($subOrder->total_sen)</span>
                                </a>
                            </li>
                        @endforeach
                    </ul>
