        ...
        @if ($affiliate === null)
            ...
            <div class="rounded-[var(--radius-card)] border border-line bg-surface p-6 shadow-soft">
                <div class="flex items-start gap-4">
                    ...
                    <div class="min-w-0">
                        <h2 class="font-display text-xl font-medium">{{ __('Earn by sharing what you love') }}</h2>
                ...
                <button type="button" wire:click="enroll" wire:loading.attr="disabled" wire:target="enroll"
                        class="mt-5 inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] bg-emerald px-5 text-sm font-medium text-white transition-colors hover:bg-emerald-deep disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald focus-visible:ring-offset-1">
        ...
        @else
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="rounded-[var(--radius-card)] border border-line bg-surface p-4 shadow-soft">
                    ...
                    <p class="mt-1 font-display text-2xl font-medium tnum">{{ number_format($affiliate->clicks) }}</p>
                </div>
                <div class="rounded-[var(--radius-card)] border border-line bg-surface p-4 shadow-soft">
                    <p class="text-[11px] font-medium uppercase tracking-[0.08em] text-ink-faint">{{ __('Referred sales') }}</p>
                    <p class="mt-1 font-display text-2xl font-medium tnum">{{ number_format($referrals->count()) }}</p>
                </div>
            </div>

            {{-- Share link --}}
            <div class="mt-5 rounded-[var(--radius-card)] border border-line bg-surface p-5 shadow-soft" x-data="{ copied: false }">
                <h2 class="font-display text-lg font-medium">{{ __('Your share link') }}</h2>
                ...
                <div class="mt-3 flex gap-2">
                    ...
                    <button type="button"
                            ...
                            class="inline-flex min-h-11 shrink-0 items-center rounded-[var(--radius-control)] border border-line-strong bg-surface px-4 text-sm font-medium text-ink hover:border-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald">
            ...
            <div class="mt-5 rounded-[var(--radius-card)] border border-line bg-surface p-5 shadow-soft">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h2 class="font-display text-lg font-medium">{{ __('Withdraw earnings') }}</h2>
                ...
                @if ($showWithdraw)
                    <form wire:submit="requestPayout" class="mt-4 grid gap-3 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <button type="submit" class="inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] bg-emerald px-5 text-sm font-medium text-white hover:bg-emerald-deep">{{ __('Submit request') }}</button>
                        </div>
                    </form>
                @endif
                    ...
                    <ul class="mt-4 divide-y divide-line border-t border-line">
                        @foreach ($payouts as $payout)
                            <li class="flex items-center justify-between gap-3 py-2.5 text-[13px]">
                                ...
                                <span class="font-medium text-ink tnum">@money($payout->amount_sen)</span>
            ...
            <div class="mt-5 rounded-[var(--radius-card)] border border-line bg-surface shadow-soft">
                <h2 class="border-b border-line px-5 py-4 font-display text-lg font-medium">{{ __('Recent commissions') }}</h2>
