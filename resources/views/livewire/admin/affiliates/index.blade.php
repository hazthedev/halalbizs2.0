<div class="space-y-4">
    ...
    <div class="overflow-hidden rounded-[var(--radius-card)] border border-line bg-surface shadow-soft">
        <h2 class="border-b border-line px-4 py-3 font-medium">{{ __('Pending withdrawals') }} <span class="text-ink-faint">({{ $pending->count() }})</span></h2>
        @if ($pending->isEmpty())
                ...
                @foreach ($pending as $payout)
                    <li wire:key="payout-{{ $payout->id }}" class="px-4 py-3">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div class="min-w-0">
                                <p class="text-sm font-medium text-ink">{{ $payout->affiliate?->user?->name }} · <span class="font-medium tnum">@money($payout->amount_sen)</span></p>
                                <p class="text-xs text-ink-faint">{{ $payout->requested_at?->format('d M Y') }} · {{ data_get($payout->bank_snapshot, 'details') }}</p>
                            </div>
                            <div class="flex items-center gap-2">
                                <button type="button" wire:click="openPay({{ $payout->id }})" class="inline-flex min-h-9 items-center rounded-[var(--radius-control)] bg-emerald px-3 text-[13px] font-medium text-white hover:bg-emerald-deep">{{ __('Mark paid') }}</button>
                                <button type="button" wire:click="openReject({{ $payout->id }})" class="text-[13px] font-medium text-ink-soft hover:text-danger">{{ __('Reject') }}</button>
                            </div>
                        </div>
                        ...
                        @if ($payingId === $payout->id)
                            <form wire:submit="markPaid" class="mt-2 flex flex-wrap items-end gap-2">
                                ...
                                <button type="submit" class="inline-flex min-h-11 items-center rounded-[var(--radius-control)] bg-emerald px-4 text-sm font-medium text-white">{{ __('Confirm paid') }}</button>
                        ...
                        @if ($rejectingId === $payout->id)
                            <form wire:submit="reject" class="mt-2 flex flex-wrap items-end gap-2">
                                ...
                                <button type="submit" class="inline-flex min-h-11 items-center rounded-[var(--radius-control)] border border-danger px-4 text-sm font-medium text-danger hover:bg-danger hover:text-white">{{ __('Confirm reject') }}</button>
    ...
    <div class="overflow-hidden rounded-[var(--radius-card)] border border-line bg-surface shadow-soft">
        <table class="w-full text-sm">
            <thead class="border-b border-line text-left text-[11px] uppercase tracking-[0.06em] text-ink-faint">
                <tr>
                    <th class="px-4 py-3 font-medium">{{ __('Creator') }}</th>
                    <th class="px-4 py-3 font-medium">{{ __('Code') }}</th>
                    <th class="px-4 py-3 text-right font-medium">{{ __('Clicks') }}</th>
                    <th class="px-4 py-3 text-right font-medium">{{ __('Referrals') }}</th>
                    <th class="px-4 py-3 text-right font-medium">{{ __('Commission') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-line">
                @forelse ($affiliates as $affiliate)
                    <tr wire:key="aff-{{ $affiliate->id }}">
                        ...
                        <td class="px-4 py-3 text-right font-medium tnum">@money((int) $affiliate->commission_sen_sum)</td>
                    </tr>
                @empty
