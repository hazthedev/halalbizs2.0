<div class="mx-auto max-w-4xl space-y-4">
    ...
    <form wire:submit="publish" class="space-y-4">
        ...
        <x-ui.card class="p-5" x-data="{ lang: 'en' }">
            <h2 class="font-display text-lg font-medium">{{ __('Basics') }}</h2>
                ...
                <div>
                    <div class="mb-1.5 flex items-center justify-between gap-2">
                        ...
                        <button type="button" wire:click="generateCopy" wire:loading.attr="disabled" wire:target="generateCopy"
                                class="inline-flex items-center gap-1 rounded-[var(--radius-control)] border border-brass/40 px-2.5 py-1 text-[12px] font-medium text-brass hover:bg-brass-tint/40">
                    ...
                    <textarea id="description.en" wire:model="description.en" rows="6"
                              class="block w-full rounded-[var(--radius-control)] border border-line-strong bg-surface px-3.5 py-2.5 text-sm text-ink placeholder:text-ink-faint focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald"
            ...
            <div class="mt-5">
                ...
                <div class="grid gap-2 sm:grid-cols-3">
                    <select wire:model.live="categoryTop" aria-label="{{ __('Top category') }}"
                            class="min-h-11 rounded-[var(--radius-control)] border border-line-strong bg-surface px-3 py-2 text-sm text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald">
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <select id="brandId" wire:model="brandId"
                            class="block min-h-11 w-full rounded-[var(--radius-control)] border border-line-strong bg-surface px-3 py-2 text-sm text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald">
                ...
                <fieldset>
                    ...
                    <div class="flex gap-2">
                        @foreach ($conditions as $conditionCase)
                            <label class="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-[var(--radius-control)] border px-4 py-2 text-sm font-medium {{ $condition === $conditionCase->value ? 'border-emerald bg-emerald-tint text-emerald' : 'border-line-strong text-ink' }}">
                ...
                <fieldset class="mt-4">
                    ...
                    <select wire:model="taxClass" class="min-h-11 w-full rounded-[var(--radius-control)] border border-line-strong bg-canvas px-3 text-sm text-ink focus:border-emerald focus:ring-emerald">
                                ...
                                <div>
                                    ...
                                    <div class="mt-1 flex flex-wrap gap-1.5">
                                        @foreach ($facetAttribute->values as $facetValue)
                                            <label class="inline-flex cursor-pointer items-center gap-1.5 rounded-[var(--radius-control)] border px-3 py-1.5 text-[13px] {{ in_array($facetValue->id, $attributeValueIds) ? 'border-emerald bg-emerald-tint text-emerald' : 'border-line-strong text-ink' }}">
        ...
        @if (config('metafields.enabled', true))
            <x-ui.card class="p-5">
                <h2 class="font-display text-lg font-medium">{{ __('Halal & product details') }}</h2>
                ...
                @foreach (config('metafields.groups', []) as $groupKey => $groupLabel)
                    ...
                    @if ($fields->isNotEmpty())
                        <fieldset class="mt-4">
                            <legend class="text-[11px] font-medium uppercase tracking-[0.06em] text-ink-faint">{{ __($groupLabel) }}</legend>
                                    ...
                                    <div @class(['sm:col-span-2' => ($def['type'] ?? 'text') === 'textarea'])>
                                        ...
                                        @if (($def['type'] ?? 'text') === 'textarea')
                                            <textarea id="mf-{{ $key }}" wire:model="metafields.{{ $key }}" rows="2" maxlength="2000"
                                                      class="mt-1 block w-full rounded-[var(--radius-control)] border border-line-strong bg-canvas px-3 py-2 text-sm text-ink focus:border-emerald focus:ring-emerald"></textarea>
                                        @else
                                            <input id="mf-{{ $key }}" type="{{ ($def['type'] ?? 'text') === 'date' ? 'date' : 'text' }}"
                                                   wire:model="metafields.{{ $key }}" maxlength="255"
                                                   class="mt-1 block min-h-11 w-full rounded-[var(--radius-control)] border border-line-strong bg-canvas px-3 text-sm text-ink focus:border-emerald focus:ring-emerald">
                                        @endif
                                    </div>
                                @endforeach
                            </div>
        ...
        {{-- ── Images ─────────────────────────────────────────────── --}}
        <x-ui.card class="p-5">
            <h2 class="font-display text-lg font-medium">{{ __('Images') }}</h2>
            ...
            <div class="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-5">
                @foreach ($existingMedia as $index => $media)
                    <div class="group relative aspect-square overflow-hidden rounded-[var(--radius-card)] border border-line bg-paper" wire:key="media-{{ $media->id }}">
                        ...
                        <div class="absolute inset-x-0 bottom-0 flex justify-between gap-1 bg-emerald-night/70 p-1">
                            <button type="button" wire:click="moveMedia({{ $media->id }}, -1)" @disabled($loop->first) class="flex size-8 items-center justify-center rounded text-on-dark disabled:opacity-40" aria-label="{{ __('Move image earlier') }}">←</button>
                            <button type="button" wire:click="removeMedia({{ $media->id }})" class="flex size-8 items-center justify-center rounded text-on-dark hover:text-danger" aria-label="{{ __('Remove image') }}">✕</button>
                            <button type="button" wire:click="moveMedia({{ $media->id }}, 1)" @disabled($loop->last) class="flex size-8 items-center justify-center rounded text-on-dark disabled:opacity-40" aria-label="{{ __('Move image later') }}">→</button>
                        </div>
                    </div>
                @endforeach
                ...
                @foreach ($newImages as $index => $image)
                    <div class="group relative aspect-square overflow-hidden rounded-[var(--radius-card)] border border-line bg-paper" wire:key="new-image-{{ $index }}-{{ $image->getFilename() }}">
                        ...
                        <div class="absolute inset-x-0 bottom-0 flex justify-between gap-1 bg-emerald-night/70 p-1">
                            <button type="button" wire:click="moveNewImage({{ $index }}, -1)" @disabled($loop->first) class="flex size-8 items-center justify-center rounded text-on-dark disabled:opacity-40" aria-label="{{ __('Move image earlier') }}">←</button>
                            <button type="button" wire:click="removeNewImage({{ $index }})" class="flex size-8 items-center justify-center rounded text-on-dark hover:text-danger" aria-label="{{ __('Remove image') }}">✕</button>
                            <button type="button" wire:click="moveNewImage({{ $index }}, 1)" @disabled($loop->last) class="flex size-8 items-center justify-center rounded text-on-dark disabled:opacity-40" aria-label="{{ __('Move image later') }}">→</button>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="mt-6 border-t border-line pt-4">
                <h3 class="text-sm font-medium text-ink">{{ __('Video') }}</h3>
                ...
                @if ($newVideo !== null)
                    <div class="mt-3 flex flex-wrap items-center gap-3">
                        @if ($newVideo->isPreviewable())
                            <video src="{{ $newVideo->temporaryUrl() }}" controls preload="metadata" class="h-28 rounded-[var(--radius-control)] border border-line bg-paper"></video>
                        @else
                ...
                @elseif ($existingVideo !== null)
                    <div class="mt-3 flex flex-wrap items-center gap-3">
                        <video src="{{ $existingVideo->getUrl() }}" controls preload="metadata" class="h-28 rounded-[var(--radius-control)] border border-line bg-paper"></video>
                ...
                @else
                    <label class="mt-3 inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-[var(--radius-control)] border border-dashed border-line-strong px-4 text-[13px] font-medium text-ink-soft hover:border-ink hover:text-ink">
        ...
        <x-ui.card class="p-5">
            <div class="flex items-center justify-between gap-3">
                <div>
                    <h2 class="font-display text-lg font-medium">{{ __('Variations') }}</h2>
            ...
            @error('matrix')
                <div class="mt-3 rounded-[var(--radius-control)] border border-danger bg-danger-tint px-3 py-2 text-[13px] text-danger">{{ $message }}</div>
            @enderror
                ...
                <div class="mt-4 space-y-3">
                        ...
                        <div class="rounded-[var(--radius-card)] border border-line p-4" wire:key="option-group-{{ $groupIndex }}">
                            <div class="flex items-end gap-2">
                                ...
                                <button type="button" wire:click="removeOptionGroup({{ $groupIndex }})"
                                        class="inline-flex min-h-11 items-center rounded-[var(--radius-control)] px-3 text-[13px] font-medium text-ink-soft hover:text-danger focus-visible:ring-2 focus-visible:ring-emerald">
                                    {{ __('Remove') }}
                                </button>
                            </div>
                            ...
                            <div class="mt-3 flex flex-wrap items-center gap-2">
                                ...
                                <div class="flex items-center gap-1">
                                    <input
                                        ...
                                        class="min-h-11 w-36 rounded-[var(--radius-control)] border border-line-strong bg-surface px-3 py-1.5 text-[13px] text-ink placeholder:text-ink-faint focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald"
                                    >
                    ...
                    @if (count($optionGroups) < \App\Livewire\Seller\Products\Form::MAX_OPTION_GROUPS)
                        <button type="button" wire:click="addOptionGroup"
                                class="inline-flex min-h-11 items-center gap-2 rounded-[var(--radius-control)] border border-ink px-4 text-sm font-medium text-ink hover:bg-paper focus-visible:ring-2 focus-visible:ring-emerald">
                                    ...
                                    <td class="px-3 py-2">
                                        <div class="flex items-center gap-1">
                                            <input type="text" wire:model="bulkPrice" inputmode="decimal" placeholder="0.00" aria-label="{{ __('Price for all variants') }}"
                                                   class="min-h-11 w-20 rounded-[var(--radius-control)] border border-line-strong bg-surface px-2 py-1.5 text-right tabular-nums focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald">
                                            <button type="button" wire:click="applyPriceToAll" class="min-h-11 rounded-[var(--radius-control)] px-2 text-[12px] font-medium text-ink-soft hover:text-ink focus-visible:ring-2 focus-visible:ring-emerald">{{ __('Set') }}</button>
                                        </div>
                                    </td>
                                    <td class="px-3 py-2">
                                        <div class="flex items-center gap-1">
                                            <input type="text" wire:model="bulkSalePrice" inputmode="decimal" placeholder="0.00" aria-label="{{ __('Sale price for all variants') }}"
                                                   class="min-h-11 w-20 rounded-[var(--radius-control)] border border-line-strong bg-surface px-2 py-1.5 text-right tabular-nums focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald">
                                            <button type="button" wire:click="applySalePriceToAll" class="min-h-11 rounded-[var(--radius-control)] px-2 text-[12px] font-medium text-ink-soft hover:text-ink focus-visible:ring-2 focus-visible:ring-emerald">{{ __('Set') }}</button>
                                        </div>
                                    </td>
                                    <td class="px-3 py-2">
                                        <div class="flex items-center gap-1">
                                            <input type="text" wire:model="bulkStock" inputmode="numeric" placeholder="0" aria-label="{{ __('Stock for all variants') }}"
                                                   class="min-h-11 w-16 rounded-[var(--radius-control)] border border-line-strong bg-surface px-2 py-1.5 text-right tabular-nums focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald">
                                            <button type="button" wire:click="applyStockToAll" class="min-h-11 rounded-[var(--radius-control)] px-2 text-[12px] font-medium text-ink-soft hover:text-ink focus-visible:ring-2 focus-visible:ring-emerald">{{ __('Set') }}</button>
                                        </div>
                                    </td>
                                    ...
                                    <tr class="border-b border-line last:border-b-0 hover:bg-paper" wire:key="matrix-{{ $key }}">
                                        ...
                                        <td class="px-3 py-2">
                                            <input type="text" wire:model="matrix.{{ $key }}.price" inputmode="decimal" placeholder="0.00" aria-label="{{ __(':label price', ['label' => $row['label']]) }}"
                                                   class="min-h-11 w-24 rounded-[var(--radius-control)] border bg-surface px-2 py-1.5 text-right tabular-nums focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald {{ $errors->has('matrix.'.$key.'.price') ? 'border-danger' : 'border-line-strong' }}">
                                        ...
                                        <td class="px-3 py-2">
                                            <input type="text" wire:model="matrix.{{ $key }}.sale_price" inputmode="decimal" placeholder="—" aria-label="{{ __(':label sale price', ['label' => $row['label']]) }}"
                                                   class="min-h-11 w-24 rounded-[var(--radius-control)] border bg-surface px-2 py-1.5 text-right tabular-nums focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald {{ $errors->has('matrix.'.$key.'.sale_price') ? 'border-danger' : 'border-line-strong' }}">
                                        ...
                                        <td class="px-3 py-2">
                                            <input type="text" wire:model="matrix.{{ $key }}.stock" inputmode="numeric" aria-label="{{ __(':label stock', ['label' => $row['label']]) }}"
                                                   class="min-h-11 w-16 rounded-[var(--radius-control)] border bg-surface px-2 py-1.5 text-right tabular-nums focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald {{ $errors->has('matrix.'.$key.'.stock') ? 'border-danger' : 'border-line-strong' }}">
                                        ...
                                        <td class="px-3 py-2">
                                            <input type="text" wire:model="matrix.{{ $key }}.sku" aria-label="{{ __(':label SKU', ['label' => $row['label']]) }}"
                                                   class="min-h-11 w-28 rounded-[var(--radius-control)] border bg-surface px-2 py-1.5 font-mono text-[12px] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald {{ $errors->has('matrix.'.$key.'.sku') ? 'border-danger' : 'border-line-strong' }}">
        ...
        {{-- ── Sale schedule ──────────────────────────────────────── --}}
        <x-ui.card class="p-5">
            <h2 class="font-display text-lg font-medium">{{ __('Sale schedule (optional)') }}</h2>
        ...
        {{-- ── Shipping ───────────────────────────────────────────── --}}
        <x-ui.card class="p-5">
            <h2 class="font-display text-lg font-medium">{{ __('Shipping') }}</h2>
