<div class="space-y-4">
    ...
    <x-ui.section-heading as="h1" :title="__('Products')">
        <x-slot:actions>
            <x-ui.button :href="route('seller.products.import')" variant="ghost">{{ __('Bulk import') }}</x-ui.button>
                            ...
                            <td class="px-3 py-2">
                                <a href="{{ route('seller.products.edit', $product) }}" wire:navigate class="flex items-center gap-3">
                                    @if ($url = $product->getFirstMediaUrl('images', 'thumb'))
                                        <img src="{{ $url }}" alt="{{ $product->getTranslation('name', 'en') }}" class="size-10 shrink-0 rounded-lg border border-line bg-paper object-contain">
                                    @else
