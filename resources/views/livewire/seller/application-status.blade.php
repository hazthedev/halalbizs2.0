    ...
    @if ($store !== null)
        @if ($store->status === \App\Enums\StoreStatus::Pending)
            <x-ui.card class="p-6 sm:p-8">
                ...
                <h1 class="mt-4 font-display text-[28px] font-medium leading-tight">{{ __('Application received') }}</h1>
                ...
                <h2 class="mt-6 text-[13px] font-medium uppercase tracking-[0.04em] text-ink-soft">{{ __('What happens next') }}</h2>
                <ol class="mt-3 space-y-3 text-sm text-ink">
                    <li class="flex gap-3">
                        <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-paper font-mono text-[11px] font-medium">1</span>
                        {{ __('Our team verifies your SSM certificate, IC and bank details.') }}
                    </li>
                    <li class="flex gap-3">
                        <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-paper font-mono text-[11px] font-medium">2</span>
                        {{ __('Reviews usually take 2–3 business days.') }}
                    </li>
                    <li class="flex gap-3">
                        <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-paper font-mono text-[11px] font-medium">3</span>
        ...
        @elseif ($store->status === \App\Enums\StoreStatus::Rejected)
            <x-ui.card class="p-6 sm:p-8">
                ...
                <h1 class="mt-4 font-display text-[28px] font-medium leading-tight">{{ __('Application rejected') }}</h1>
                ...
                @if ($store->rejection_reason)
                    <div class="mt-4 rounded-[var(--radius-control)] border border-danger/30 bg-danger-tint p-4">
                        <p class="text-[13px] font-medium text-danger">{{ __('Reason') }}</p>
        ...
        @elseif ($store->status === \App\Enums\StoreStatus::Suspended)
            <x-ui.card class="p-6 sm:p-8">
                ...
                <h1 class="mt-4 font-display text-[28px] font-medium leading-tight">{{ __('Your shop is suspended') }}</h1>
                ...
                @if ($store->rejection_reason)
                    <div class="mt-4 rounded-[var(--radius-control)] border border-warn/30 bg-warn-tint p-4">
                        <p class="text-[13px] font-medium text-warn">{{ __('Reason') }}</p>
                        <p class="mt-1 text-sm text-ink">{{ $store->rejection_reason }}</p>
                    </div>
                @endif
