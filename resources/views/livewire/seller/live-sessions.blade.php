<div class="mx-auto w-full max-w-5xl px-4 py-6 lg:py-8">
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="font-display text-2xl font-medium">{{ __('Live shopping') }}</h1>
    ...
    @if ($showForm)
        <form wire:submit="save" class="mt-5 grid gap-4 rounded-[var(--radius-card)] border border-line bg-surface p-5 shadow-soft sm:grid-cols-2">
            ...
            <div class="flex gap-2 sm:col-span-2">
                <button type="submit" class="inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] bg-emerald px-5 text-sm font-medium text-white hover:bg-emerald-deep">{{ __('Create') }}</button>
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if ($session->status === \App\Enums\LiveSessionStatus::Live)
                            ...
                            <button type="button" wire:click="end({{ $session->id }})" class="text-[13px] font-medium text-ink-soft hover:text-danger">{{ __('End') }}</button>
                        @elseif ($session->status === \App\Enums\LiveSessionStatus::Scheduled)
                            <button type="button" wire:click="goLive({{ $session->id }})" class="inline-flex min-h-9 items-center rounded-[var(--radius-control)] bg-emerald px-3 text-[13px] font-medium text-white hover:bg-emerald-deep">{{ __('Go live') }}</button>
                        @endif
                        <button type="button" wire:click="manage({{ $session->id }})" class="text-[13px] font-medium text-ink-soft hover:text-ink">{{ $managing?->id === $session->id ? __('Close') : __('Manage rail') }}</button>
                    </div>
                </div>
                ...
                @if ($managing?->id === $session->id)
                    <div class="border-t border-line px-5 py-4">
                        ...
                        <div class="flex flex-wrap items-end gap-2">
                            ...
                            <button type="button" wire:click="addProduct" class="inline-flex min-h-11 items-center rounded-[var(--radius-control)] border border-line-strong bg-surface px-4 text-sm font-medium text-ink hover:border-ink">{{ __('Add') }}</button>
                        </div>
                                    ...
                                    <li class="flex items-center justify-between gap-3 py-2.5">
                                        ...
                                        <div class="flex items-center gap-3">
                                            @if ($managing->featured_product_id === $product->id)
                                                <span class="text-[11px] font-medium uppercase tracking-[0.06em] text-emerald">{{ __('Featured') }}</span>
                                            @else
                                                <button type="button" wire:click="feature({{ $managing->id }}, {{ $product->id }})" class="text-[13px] font-medium text-ink-soft hover:text-emerald">{{ __('Feature') }}</button>
                                            @endif
                                            <button type="button" wire:click="removeProduct({{ $managing->id }}, {{ $product->id }})" class="text-[13px] font-medium text-ink-soft hover:text-danger">{{ __('Remove') }}</button>
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
