...
Full specs live in `docs/` — see the doc map at the bottom. **Read `docs/00-overview.md` before your first task.**

## Stack

- Laravel 12.x · PHP 8.4 · MySQL 8 · Redis (cache/session/queue, Horizon)
- Livewire 4 + Alpine.js + Blade · Tailwind CSS v4 (tokens in `resources/css/app.css`) · Vite
- Scout + Meilisearch · spatie: permission, medialibrary, translatable, sluggable, activitylog, settings, backup
- brick/money · Pest · Pint · Larastan

## Commands

```bash
composer dev                # serve + queue + vite (composer script)
...
- Livewire namespaces mirror contexts: `App\Livewire\Storefront\*`, `App\Livewire\Seller\*`, `App\Livewire\Admin\*`. Shared UI in `App\Livewire\Components\*` + Blade components in `resources/views/components`.
...
Migrations run clean on `migrate:fresh --seed` · Pest green · Pint clean · Larastan clean · UI matches `docs/03-design-system.md` tokens · mobile-checked at 390px · focus states visible.

## Reference codebase

`reference/bagisto/` (gitignored shallow clone, MIT) is a mature Laravel ecommerce implementation. When implementing complex areas (voucher locking, exchange rates, returns), **read their approach, write ours** — never copy structure wholesale; our architecture decisions in `docs/` win.

## Doc map

| Doc | Use when |
|---|---|
| `docs/00-overview.md` | Start here — vision, locked decisions, build order |
| `docs/01-requirements.md` | What any feature must do (functional source of truth) |
| `docs/02-bagisto-feature-analysis.md` | Why features exist/don't; schema deltas; engineering lessons |
| `docs/03-design-system.md` | Every UI decision — tokens, components, motion, microcopy |
| `docs/04-foundation-plan.md` | Migrations, models, seeders, enums, route skeleton (M1) |
| `docs/05-storefront-plan.md` | Buyer-facing pages (M2) |
| `docs/06-checkout-payments-plan.md` | Checkout, COD, iPay88, order lifecycle (M4–M6) |
| `docs/07-seller-panel-plan.md` | Seller centre (M3, M6) |
| `docs/08-admin-panel-plan.md` | Admin panel (M7) |
| `docs/09-marketplace-depth-plan.md` | Reviews, vouchers, returns, seller finance (M8) |
| `docs/10-deployment-ops.md` | Infra, scheduler, env, go-live checklists |
